...
class DataGroup:
    ...
    def load_data(self, data, keys) -> Dict[str, Any]:
        ...
        if isinstance(data, zarr.hierarchy.Group):
            self._root = data
            data = {k: data.get(k, None) for k in keys}
        return data
        ...
        if self.strict:
            ...
            for key in self._data:
                if n is None:
                    ...
                    assert n == self._data[key].shape[0]
    ...
    @classmethod
    def load_from_npz(cls, npz_file, keys=None):
        """Load a data group from an NPZ file."""
        data = np.load(npz_file, allow_pickle=True)
        if keys is not None:
            keys = keys
        else:
            keys = data.files
        group_data = {key: data[key] for key in keys}
